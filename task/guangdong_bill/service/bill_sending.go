...
func (bs *billSending) Send(limit int) {
	...
	userList := model.NewUsersGd().GetUsersGdById(id, limit)
...
func (bs *billSending) SendTest() {
	var userList []sanhe_gdmobile.TUsersGd
	userList = append(userList, sanhe_gdmobile.TUsersGd{
		Id:        0,
		Mobile:    `13535491163`,
		Uid:       `2088412520197580`,
		NewMobile: `13535491163`,
	})
	bs.SendData(userList, len(userList))
}

func (bs *billSending) SendData(userList []sanhe_gdmobile.TUsersGd, userNum int) {
	// 发送小程序模板
...
func (bs *billSending) SendMobileData(userList []sanhe_gdmobile.TUsersGd, userNum int) {
	// 发送小程序模板
	successNum, failUser, err := bs.SendTemplateMessage(userList, userNum, 1)
	if err != nil {
		logger.Use(`bill` + CommonKey.KeyNum).Info(err.Error())
...
func (bs *billSending) Statistics(num int) {
	...
	redisCache := bs.GetCache()
	_ = redisCache.HIncrBy(key, CommonKey.KeyNum, int64(num)).Val()
...
// 设置最后查询到的ID
func (bs *billSending) setRedisId(user sanhe_gdmobile.TUsersGd) {
	key := fmt.Sprintf(config.Key.BillKey.TemplateBillKey, CommonKey.KeyNum, SendTime.Format(`200601`))
	redisCache := bs.GetCache()
...
// 发送模板消息
//sendType 1.广东移动小程序,2.广东移动生活号
func (bs *billSending) SendTemplateMessage(userList []sanhe_gdmobile.TUsersGd, userNum, sendType int) (successNum int, failUser []sanhe_gdmobile.TUsersGd, err error) {
	var (
	...
	counter := 0
	failUser = make([]sanhe_gdmobile.TUsersGd, 0)
