...
var (
	...
	CommonKey         = config.Key.BillKey.CommonKey[`sd`]
)
...
func (bs *billSending) SendTest() {
	var userList []sanhe_gdmobile.TUsersSd
	userList = append(userList, sanhe_gdmobile.TUsersSd{
		Id:        0,
		Mobile:    `13570929276`,
...
func (bs *billSending) SendData(userList []sanhe_gdmobile.TUsersSd, userNum int) {
	// 发送小程序模板
...
// 设置最后查询到的ID
func (bs *billSending) setRedisId(user sanhe_gdmobile.TUsersSd) {
	key := fmt.Sprintf(config.Key.BillKey.TemplateBillKey, CommonKey.KeyNum, SendTime.Format(`200601`))
	redisCache := bs.GetCache()
...
func (bs *billSending) SendTemplateMessage(userList []sanhe_gdmobile.TUsersSd, userNum, sendType int) (successNum int, failUser []sanhe_gdmobile.TUsersSd, err error) {
	var (
	...
	counter := 0
	failUser = make([]sanhe_gdmobile.TUsersSd, 0)
