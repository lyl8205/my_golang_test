...
import (
	...
	"go_test/task/beijing_bill/model"
	"log"
	"strconv"
	"time"
...
var (
	...
	CommonKey         = config.Key.BillKey.CommonKey[`bj`]
)
...
func (bs *billSending) SendTest() {
	var userList []sanhe_gdmobile.TUsersBj
	userList = append(userList, sanhe_gdmobile.TUsersBj{
		Id:        0,
		Mobile:    `13729003060`,
		Uid:       `2088712993546406`,
		NewMobile: `13729003060`,
	})
	bs.SendData(userList, len(userList))
}

func (bs *billSending) SendData(userList []sanhe_gdmobile.TUsersBj, userNum int) {
	// 发送小程序模板
...
// 设置最后查询到的ID
func (bs *billSending) setRedisId(user sanhe_gdmobile.TUsersBj) {
	key := fmt.Sprintf(config.Key.BillKey.TemplateBillKey, CommonKey.KeyNum, SendTime.Format(`200601`))
	redisCache := bs.GetCache()
...
func (bs *billSending) SendTemplateMessage(userList []sanhe_gdmobile.TUsersBj, userNum, sendType int) (successNum int, failUser []sanhe_gdmobile.TUsersBj, err error) {
	var (
	...
	for i := 0; i < corNumInt; i++ {
		...
		go func(start, end int) {
			...
			for _, v := range user {
				...
				reqParam.PageUrl = reqParam.PageUrl + v.Mobile
				switch sendType {
				case 1:
	...
	counter := 0
	failUser = make([]sanhe_gdmobile.TUsersBj, 0)
